...
var mongoDB = 'mongodb://localhost:27017/library'; //directly added it here
...
function authorCreate(first_name, family_name, d_birth, d_death, cb) {
  ...
  author.save(function (err) {
    if (err) {
	  console.log('Error in Author :'+ author);
      cb(err, null);
      return;
    }
...
function genreCreate(name, cb) {
  ...
  genre.save(function (err) {
    if (err) {
	  console.log('Error in Genre :'+ genre);
      cb(err, null);
      return;
    }
...
function bookCreate(title, summary, isbn, author, genre, cb) {
  ...
  book.save(function (err) {
    if (err) {
	  console.log('Error in Book :'+ book);
      cb(err, null);
      return;
    }
